...
[AddComponentMenu("UITween/UITween Runtime")]
public class UITween_TweenRuntime : MonoBehaviour 
{
	#region<Play Setting>
	[System.Serializable]
	...
	[HideInInspector][SerializeField]List<UITween_Tween> tweens = new List<UITween_Tween>();
	public UITween_Tween firstTween
	{
		get
		{
			if(tweens.Count > 0)
	...
	void Init ()
	{
		foreach (UITween_Tween thisTween in tweens)
		{
	...
	{
		UITween_Tween thisFirstTween = firstTween;
    ...
    public UITween_Tween GetTween (int index)
    {
        if (index < tweens.Count && index >= 0)
        {
            return tweens[index];
